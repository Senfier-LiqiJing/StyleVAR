...
CKPT="${1:-${CKPT:-${SCRIPT_DIR}/ckpt/grpo-best.pth}}"
OUT="${2:-${OUT:-${SCRIPT_DIR}/eval_out}}"

# If $1 starts with '--', it's a flag, not a ckpt path — restore defaults
if [[ "${CKPT}" == --* ]]; then
    shift 0  # keep positional args
    CKPT="${SCRIPT_DIR}/ckpt/grpo-best.pth"
else
...
python -u "${SCRIPT_DIR}/eval_grpo.py" \
    --ckpt          "${CKPT}" \
    --vae_ckpt      "${SCRIPT_DIR}/ckpt/vae_ch160v4096z32.pth" \
    --clip_local_dir "${SCRIPT_DIR}/ckpt/clip-vit-base-patch32" \
    --omnistyle_root  "${SCRIPT_DIR}/data/OmniStyle-150k" \
    --imagepulse_root "${SCRIPT_DIR}/data/ImagePulse" \
    --coco_root       "${SCRIPT_DIR}/data/coco2017/images/train2017" \
    --wikiart_root    "${SCRIPT_DIR}/data/wikiart" \

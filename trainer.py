...
import dist
from models import VAR, VQVAE, VectorQuantizer2, StyleVAR
...
class StyleVARTrainer(object):
    def __init__(
        ...
        super(StyleVARTrainer, self).__init__()
        self.var, self.vae_local, self.quantize_local = var, vae_local, vae_local.quantize
        self.var_wo_ddp: StyleVAR = var_wo_ddp  # after torch.compile
        self.var_opt = var_opt
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        training = self.var_wo_ddp.training
        self.var_wo_ddp.eval()
        for target_B3HW, style_B3HW, content_B3HW in ld_val:
            B, V = target_B3HW.shape[0], self.vae_local.vocab_size
            target_B3HW = target_B3HW.to(dist.get_device(), non_blocking=True)
            style_B3HW = style_B3HW.to(dist.get_device(), non_blocking=True)
            content_B3HW = content_B3HW.to(dist.get_device(), non_blocking=True)

            gt_idx_Bl: List[ITen] = self.vae_local.img_to_idxBl(target_B3HW)
            gt_BL = torch.cat(gt_idx_Bl, dim=1)
            x_BLCv_wo_first_l: Ten = self.quantize_local.idxBl_to_var_input(gt_idx_Bl)

            style_idx_Bl: List[ITen] = self.vae_local.img_to_idxBl(style_B3HW)
            content_idx_Bl: List[ITen] = self.vae_local.img_to_idxBl(content_B3HW)
            style_BLCvae = self.quantize_local.msBllist_to_BlCvae(style_idx_Bl)
            content_BLCvae = self.quantize_local.msBllist_to_BlCvae(content_idx_Bl)

            self.var_wo_ddp.forward
            logits_BLV = self.var_wo_ddp(
                x_BLCv_wo_first_l=x_BLCv_wo_first_l,
                style_BLCvae=style_BLCvae,
                content_BLCvae=content_BLCvae,
                style_img=style_B3HW,
                content_img=content_B3HW,
            )
            L_mean += self.val_loss(logits_BLV.data.view(-1, V), gt_BL.view(-1)) * B
            L_tail += self.val_loss(logits_BLV.data[:, -self.last_l:].reshape(-1, V), gt_BL[:, -self.last_l:].reshape(-1)) * B
            acc_mean += (logits_BLV.data.argmax(dim=-1) == gt_BL).sum() * (100 / gt_BL.shape[1])
    ...
    def load_state_dict(self, state, strict=True, skip_vae=False):
        for k in ('var_wo_ddp', 'vae_local', 'var_opt'):
            ...
            if m is not None:
                ...
                if ret is not None:
                    missing, unexpected = ret
                    print(f'[StyleVARTrainer.load_state_dict] {k} missing:  {missing}')
                    print(f'[StyleVARTrainer.load_state_dict] {k} unexpected:  {unexpected}')
        ...
        if config is not None:
            for k, v in self.get_config().items():
                if config.get(k, None) != v:
                    err = f'[StyleVAR.load_state_dict] config mismatch:  this.{k}={v} (ckpt.{k}={config.get(k, None)})'
                    if strict:

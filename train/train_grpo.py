...
# ---------------------------------------------------------------------------
# Add project root to path (this file lives under train/)
# ---------------------------------------------------------------------------
...
from utils.lora import (
    LoRALinear, apply_lora, set_lora_enabled,
    merge_lora_and_reinit, snapshot_lora, restore_lora,
    _extract_state_dict, bake_lora_into_plain, apply_lora_to_plain_state,
)
...
# ======================== LoRA (moved to utils.lora) =======================
# LoRALinear, apply_lora, set_lora_enabled, snapshot_lora, restore_lora,
# merge_lora_and_reinit are imported from utils.lora at the top of this file.
# We keep a thin wrapper around apply_lora to also freeze the full model and
# print the parameter-count summary that this training script expects.

def _inject_lora_and_freeze(model: StyleVAR, rank: int, alpha: float):
    # Freeze everything first (apply_lora only freezes the base weights it wraps).
    for p in model.parameters():
        p.requires_grad_(False)
    lora_params = apply_lora(model, rank, alpha)
...
def _build_model(args):
    ...
    if args.var_ckpt and os.path.exists(args.var_ckpt):
        ...
        from utils.lora import load_var_state_strict
        load_var_state_strict(model, st, ckpt if isinstance(ckpt, dict) else {})
...
def main():
    ...
    if args.use_lora:
        ...
        lora_params = _inject_lora_and_freeze(model, args.lora_rank, args.lora_alpha)

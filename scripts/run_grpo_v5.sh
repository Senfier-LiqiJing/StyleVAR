...
LOG_FILE="${ROOT}/grpo_v5.log"
OUTPUT_DIR="${ROOT}/grpo_output_v5"
...
nohup python -u "${ROOT}/train/train_grpo.py" \
    --paired_data \
    --old_data_path "${ROOT}/data/OmniStyle-150k" \
    --new_data_path "${ROOT}/data/ImagePulse" \
    --vae_ckpt "${ROOT}/ckpt/vae_ch160v4096z32.pth" \
    --var_ckpt "${ROOT}/ckpt/sft-best.pth" \
    --out_dir "${OUTPUT_DIR}" \
